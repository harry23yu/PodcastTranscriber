...
export default function TranscriptDisplay({ title, date, creator, duration, text, utterances, showTimestamps }) {
    ...
    const handleDownloadPDF = () => {
        ...
        // Old if statement (timestamps only appear in PDF if user checks the "Show timestamps" box, but if timestamps appear, then ads will be in the PDF, and if there are no timestamps, ads won't be in the PDF)
        // if (text && text.trim().length > 0) {
        ...
        // New if statement (ads need to never appear in the PDF, regardless of timestamps are turned on)
